### independent_targets.R --- 
#----------------------------------------------------------------------
## Author: Thomas Alexander Gerds
## Created: May  5 2022 (11:08) 
## Version: 
## Last-Updated: May 31 2022 (15:16) 
...
library(data.table)
independent_fixed <- list(horizon = 5,
                          event.times = c("T1","T2"),
...
independent_fixed_target <- tar_target(INDEPENDENT_FIXED,independent_fixed,deployment = "main")
independent_varying <- data.table::CJ(A1_T1 = c(1,1.25,1.5),
                                      A1_T2 = 1,
                                      A2_T1 = 1,
                                      A2_T2 = 1.25,
                                      scale.censored = c(-Inf,1/40,1/25),
                                      sample.size = c(500,1000,2000,5000))
independent_varying_target <- tar_target(INDEPENDENT_VARYING,independent_varying,deployment = "main")
independent_estimators <- rbindlist(list(data.table::CJ(num.trees = c(100),
                                                        net = FALSE,
...
independent_estimators_target <- tar_target(INDEPENDENT_ESTIMATORS,independent_estimators,deployment = "main")
...
independent_truth_varying <- tar_map(
    # truth is affected by % censored and true effect
    values = independent_varying[,.(scale.censored,A1_T1,A1_T2,A2_T1,A2_T2)],
    tar_target(VARYING_TRUTH,{
        lavaModel;
        simulateData;
        y = theTruth(setting = INDEPENDENT_FIXED,
                     A1_T1 = A1_T1,
                     A1_T2 = A1_T2,
                     A2_T1 = A2_T1,
                     A2_T2 = A2_T2,
                     scale.censored = scale.censored,
                     horizon = INDEPENDENT_FIXED$horizon,
                     n = 1000000)
        y = cbind(y, A1_T1 = A1_T1,A1_T2 = A1_T2, A2_T1 = A2_T1,A2_T2 = A2_T2)
        y},
        deployment = "worker"))

independent_truth <- tar_combine(INDEPENDENT_TRUTH,
                                 independent_truth_varying)
# ---------------------------------------------------------------------
# Estimation of ATE 
# ---------------------------------------------------------------------
independent_estimates <- tar_map(
    # outer map generates data under varying parameters of the data generating model
    values = independent_varying[,.(A1_T1,A1_T2,A2_T1,A2_T2,scale.censored,sample.size)],
    unlist = FALSE,
    tar_target(IND_SIM_DATA,{
        lavaModel
        simulateData(setting = INDEPENDENT_FIXED,
                     A1_T1 = A1_T1,
                     A1_T2 = A1_T2,
                     A2_T1 = A2_T1,
                     A2_T2 = A2_T2,
    ...
    inner_independent <- tar_map(
        values = independent_estimators[,.(net,num.trees,method,weighter)],
        unlist = FALSE,
        tar_target(ESTIMATE,{
            if(weighter == "km")
            ...
            x <- causalhunter(formula=Hist(time,event)~intervene(A1)+intervene(A2)+A3+A4+A5+A6+A7+A8+A9+A10+X1+X2+X3+X4+X5+X6+X7,
                              ...
                              CR.as.censoring = net,
                              data=IND_SIM_DATA,
                              times=INDEPENDENT_FIXED$horizon,
                              formula.weight = ff)
        ...
        deployment = "worker",
        storage = "worker",
        pattern = map(IND_SIM_DATA))
    )
)
# combine
independent_ate <- tar_combine(INDEPENDENT_ESTIMATE_ATE,{
...
# ---------------------------------------------------------------------
independent_results <-
    tar_target(INDEPENDENT_RESULTS,
               summarizePerformance(truth = INDEPENDENT_TRUTH,
                                    estimate = INDEPENDENT_ESTIMATE_ATE),
               deployment = "main")

independent_boxplots <- tar_target(INDEPENDENT_BOXPLOTS,{
    e <- INDEPENDENT_ESTIMATE_ATE[intervene == "A1"]
    t <- INDEPENDENT_TRUTH
    e <- tar_read("INDEPENDENT_ESTIMATE_ATE")[intervene == "A1"]
    t <- tar_read("INDEPENDENT_TRUTH")[intervene == "A1"]
    setkeyv(e,c("net","intervene","A1_T1","A1_T2","A2_T1","A2_T2"))
    setkeyv(t,c("net","intervene","A1_T1","A1_T2","A2_T1","A2_T2"))
    t.ate = t[cause == 1 & intervene == "A1",.(true.ate = mean(ate)),keyby = c("net","intervene","A1_T1","A1_T2","A2_T1","A2_T2")]
    ...
    g <- ggplot(e,aes(x = n,y = ate))
    g <- g+geom_boxplot()
    g <- g+geom_hline(aes(yintercept = true.ate,color = "red"),data = e)
    g+facet_grid(scale.censored~A1_T1)
})
independent_coverage <- tar_target(INDEPENDENT_COVERAGE,{
    x <- INDEPENDENT_RESULTS
...
# Plot of setting
# ---------------------------------------------------------------------
independent_plot_setting <- tar_map(
    values = independent_varying[,.(scale.censored,A1_T1,A1_T2,A2_T1,A2_T2)],
    tar_target(PLOT_INDEPENDENT,
               plotTruth(setting = INDEPENDENT_FIXED,
                         A1_T1 = A1_T1,
                         A1_T2 = A1_T2,
                         A2_T1 = A2_T1,
                         A2_T2 = A2_T2,
...
######################################################################
### independent_targets.R ends here

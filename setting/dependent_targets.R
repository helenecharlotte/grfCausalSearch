...
## Created: May  5 2022 (11:09) 
## Version: 
## Last-Updated: May 19 2022 (10:09) 
...
### Code:
# dependent censoring
dependent_fixed <- list(horizon = 3,
                        event.times = c("T1","T2"),
...
dependent_fixed_target <- tar_target(DEPENDENT_FIXED,dependent_fixed,deployment = "main")
dependent_varying <- data.table::CJ(A1_T1 = c(1,1.25,1.5),
                                    A1_T2 = 1,
                                    A2_T1 = 1,
                                    A2_T2 = 1.25,
                                    scale.censored = 1/25, # c(-Inf,1/40,1/25),
                                    sample.size = c(5000))
dependent_varying_target <- tar_target(DEPENDENT_VARYING,
                                       dependent_varying,
                                       deployment = "main")
dependent_estimators <- rbindlist(list(data.table::CJ(num.trees = c(100),
                                                      net = c(FALSE),
                                       ...
                                       data.table::CJ(num.trees = c(100),
                                                      ...
                                                      method = c("naive"),
                                                      weighter = c("km"))))
dependent_estimators_target <- tar_target(DEPENDENT_ESTIMATORS,dependent_estimators,deployment = "main")
...
dependent_truth_varying <- tar_map(
    # truth is affected by % censored and true effect
    values = dependent_varying[,.(scale.censored,A1_T1,A1_T2,A2_T1,A2_T2)],
    tar_target(DEPENDENT_VARYING_TRUTH,{
        lavaModel;
        simulateData;
        y = theTruth(setting = DEPENDENT_FIXED,
                     A1_T1 = A1_T1,
                     A1_T2 = A1_T2,
                     A2_T1 = A2_T1,
                     A2_T2 = A2_T2,
                     scale.censored = scale.censored,
                     horizon = DEPENDENT_FIXED$horizon,
                     n = 1000000)
        y = cbind(y, A1_T1 = A1_T1,A1_T2 = A1_T2, A2_T1 = A2_T1,A2_T2 = A2_T2)
        y},
        deployment = "worker"))

dependent_truth <- tar_combine(DEPENDENT_TRUTH,
                                 dependent_truth_varying)
# ---------------------------------------------------------------------
# Estimation of ATE 
# ---------------------------------------------------------------------
dependent_estimates <- tar_map(
    # outer map generates data under varying parameters of the data generating model
    values = dependent_varying[,.(A1_T1,A1_T2,A2_T1,A2_T2,scale.censored,sample.size)],
    unlist = FALSE,
    tar_target(DEPENDENT_SIM_DATA,{
        lavaModel
        simulateData(setting = DEPENDENT_FIXED,
                     A1_T1 = A1_T1,
                     A1_T2 = A1_T2,
                     A2_T1 = A2_T1,
                     A2_T2 = A2_T2,
    ...
    inner_dependent <- tar_map(
        values = dependent_estimators[,.(net,num.trees,method,weighter)],
        unlist = FALSE,
        tar_target(DEPENDENT_ESTIMATE,{
            if(weighter == "km")
            ...
            x <- causalhunter(formula=Hist(time,event)~intervene(A1)+intervene(A2)+A3+A4+A5+A6+A7+A8+A9+A10+X1+X2+X3+X4+X5+X6+X7,
                              method = method,
                              weighter=weighter,
                              args.weight = list(num.trees = 50),
                              num.trees=num.trees,
                              CR.as.censoring = net,
                              data=DEPENDENT_SIM_DATA,
                              times=DEPENDENT_FIXED$horizon,
                              formula.weight = ff)
        ...
        deployment = "worker",
        pattern = map(DEPENDENT_SIM_DATA))
    )
)
# combine
dependent_ate <- tar_combine(DEPENDENT_ESTIMATE_ATE,{
    # the first element are the data
    dependent_estimates[-1]
})
# ---------------------------------------------------------------------
# Summarize performance of estimators against true parameter values
# ---------------------------------------------------------------------
dependent_results <-
    tar_target(DEPENDENT_RESULTS,
               summarizePerformance(truth = DEPENDENT_TRUTH,
                                    estimate = DEPENDENT_ESTIMATE_ATE),
               deployment = "main")

dependent_boxplots <- tar_target(DEPENDENT_BOXPLOTS,{
    e <- DEPENDENT_ESTIMATE_ATE[intervene == "A1"]
    t <- DEPENDENT_TRUTH
...
dependent_coverage <- tar_target(DEPENDENT_COVERAGE,{
    x <- DEPENDENT_RESULTS
...
# Plot of setting
# ---------------------------------------------------------------------
dependent_plot_setting <- tar_map(
    values = dependent_varying[,.(scale.censored,A1_T1,A1_T2,A2_T1,A2_T2)],
    tar_target(PLOT_DEPENDENT,
               plotTruth(setting = DEPENDENT_FIXED,
                         A1_T1 = A1_T1,
                         A1_T2 = A1_T2,
                         A2_T1 = A2_T1,
                         A2_T2 = A2_T2,
